...
4. Run the experiment. Commit per cycle in the code repo: `r/<slug> cycle N: <change> -> <metric or obs>. qual: <one line>.` Push after each cycle. Analysis-only cycles get `git commit --allow-empty`. **Do not commit the paper per cycle by default** — cycle lines are batched into one paper commit at step 5. Exception: for moves longer than ~30 minutes, append a `Since last update` line per cycle so the human gets live progress; this is the only time per-cycle paper commits are warranted.
   - Mechanical shortcut: `vr-research-runner <project-dir> run --cwd <code-repo> --command "<cmd>" --metric-regex "<regex>" --seed <id> --git-commit --wait-human` claims a queued move, records a cycle artifact, captures seed/git provenance, optionally commits/pushes the code repo, creates the fast human review card, and waits for a click when you need a gate. Add `--agent-review-provider <id>` to launch a reviewer agent session that can inspect the card evidence and resolve it, and add `--monitor-url <url>` for long cycles with W&B/TensorBoard/Modal monitors so the live run appears on the agent canvas. Use `--ask-human` instead when the agent should keep moving after posting the card. Use `vr-research-judge <project-dir> --slug <slug> --ask-human` after meaningful cycles or finish to audit doctor/admit/paper state and ask for `continue` / `rerun` / `synthesize` / `brainstorm` / `steer`. When the move is done, `vr-research-runner <project-dir> finish --slug <slug> --aggregate-metric --auto-admit --update-paper --publish-canvas --cost-compute <N> --cost-dollars <N> --apply` writes mean/std frontmatter, pastes the admission verdict, records/debits move cost, updates the paper/canvas, removes ACTIVE, applies Queue updates, and appends LOG through `vr-research-resolve`.
...
- Create tiny user-facing action items with `vr-agent-ask` or `POST $VIBE_RESEARCH_AGENT_TOWN_API/action-items`.
- Use action item metadata when it matters: `kind` (`action`, `approval`, `review`, `setup`), `priority` (`low`, `normal`, `high`, `urgent`), `sourceSessionId`, `target`, `capabilityIds`, `recommendation`, `consequence`, `evidence`, and `choices`.
- For human review, prefer one sharp card with `choices: ["approve","reject","steer"]`; then wait on `action_item_resolved`, `action_item_approved`, `action_item_rejected`, or `action_item_steered`.
- For phase routing, start with `vr-research-orchestrator tick <project-dir> --ask-human`; it reads research phase, ACTIVE, QUEUE, LOG, doctor, planned/running `runs.tsv` sweeps, and the latest result, then recommends the next runner / sweep / brief / judge action. Use `--apply` only for safe phase transitions such as exhausted `experiment` -> `review`, or to compile an already-reviewed brief into QUEUE.
- For brainstorm-to-experiment transitions, write a durable brief with `vr-research-brief <project-dir> create ... --ask-human`; the Agent Inbox card can `approve & queue` in one click, and terminal agents can still compile the selected candidate with `vr-research-brief <project-dir> compile --slug <brief>` instead of hand-editing QUEUE.
- For queued experiments, use `vr-research-runner <project-dir> run --cwd <code-repo> --command "<cmd>" --metric-regex "<regex>" --seed <id> --git-commit --wait-human` to claim the move, record a cycle, capture artifacts, and ask for `continue` / `rerun` / `synthesize` / `brainstorm` / `steer` review while waiting when a human gate is needed. Add `--agent-review-provider <id>` to launch a reviewer agent session that can inspect evidence and resolve the card, and add `--monitor-url <url>` for long cycles with W&B/TensorBoard/Modal monitors so the live run appears on the agent canvas. Use `--ask-human` instead when the agent should keep moving after posting the card. Use `vr-research-judge <project-dir> --slug <slug> --ask-human` after meaningful cycles or finish to audit doctor/admit/paper state and surface the next action. Use `vr-research-runner <project-dir> finish --slug <slug> --aggregate-metric --auto-admit --update-paper --publish-canvas --cost-compute <N> --cost-dollars <N> --apply` to set STATUS, write mean/std frontmatter, record/debit move cost, update the paper/canvas, apply Queue updates, remove ACTIVE, and log the resolution.
...
- Supported predicates: `first_building_placed`, `cosmetic_building_placed` (optionally scoped with `itemId`), `functional_building_placed` (optionally scoped with `pluginId`), `agent_clicked`, `automation_created`, `library_note_saved`, `workspace_selected`, `action_item_completed` (optionally scoped with `actionItemId`), `action_item_dismissed`, `action_item_resolved`, `action_item_approved`, `action_item_rejected`, `action_item_steered`, `action_item_paused`, `action_item_resumed`, `action_item_rerun`, `action_item_continued`, `action_item_brainstorm`, `action_item_synthesized`, and `onboarding_complete`.
