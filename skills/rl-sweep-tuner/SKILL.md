...
- `vr-research-doctor projects/<name>` — validate the project's bookkeeping is consistent (links resolve, leaderboard isn't lying about commits, etc.). Also walks `runs.tsv` and `runs/<slug>.tsv`: surfaces stale `running` rows (>24h with no progress = orphaned, mark `failed` or rerun), config-JSON parse errors, missing required columns, and `running` rows with no matching ACTIVE README claim. Run this before pushing the Library.
