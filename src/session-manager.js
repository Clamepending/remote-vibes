...
export function buildSessionEnv(
  ...
  stateDir = getRemoteVibesStateDir({ cwd: workspaceRoot }),
) {
  return {
    ...process.env,
    COLORTERM: "truecolor",
    ...
    REMOTE_VIBES_AGENT_PROMPT_PATH: path.join(stateDir, AGENT_PROMPT_FILENAME),
...
async function listCodexTrackedSessions(homeDir = os.homedir()) {
  ...
  for (const sourceEntries of await Promise.all([
    ...
    for (const entry of sourceEntries) {
      ...
      const existingUpdated = mergedEntries.get(entry.id) || 0;
      const nextUpdated = Number(entry.updated || 0);
      if (nextUpdated >= existingUpdated) {
        mergedEntries.set(entry.id, nextUpdated);
      }
    }
  }
...
export class SessionManager {
  ...
  async prepareProviderLaunch(session, provider, { restored = false } = {}) {
    ...
    if (provider.id === "claude") {
      ...
      this.setPendingProviderCapture(session, null);
      const createCommand = buildShellCommand(launchCommand, ["--session-id", session.id]);
      return {
        commandString: restored
          ? buildFallbackCommand([
              buildShellCommand(launchCommand, ["--resume", session.id]),
              createCommand,
            ])
          : createCommand,
        afterLaunch: null,
    ...
    if (provider.id === "codex") {
      ...
      if (restored) {
        this.setPendingProviderCapture(session, null);
        const resumeCommand = session.providerState?.sessionId
          ? buildShellCommand(launchCommand, ["resume", session.providerState.sessionId])
          : null;
      ...
      const baselineSessionIds = new Set(
        (await listCodexTrackedSessions(this.userHomeDir)).map((entry) => entry.id),
      );
    ...
    const knownSessions = matchOpenCodeSessionsByCwd(
      await listOpenCodeSessions(
        ...
        session.cwd,
        buildSessionEnv(session.id, provider.id, this.providers, this.cwd, this.stateDir),
      ),
      session.cwd,
    );
  ...
  startSession(session, provider, { restored = false } = {}) {
    ...
    const ptyProcess = pty.spawn(session.shell, getShellArgs(session.shell), {
      cwd: sessionCwd,
      env: buildSessionEnv(session.id, provider.id, this.providers, this.cwd, this.stateDir),
      name: "xterm-256color",
      cols: session.cols,
      rows: session.rows,
    });
