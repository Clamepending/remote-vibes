...
export async function createVibeResearchApp({
  ...
  app.patch("/api/settings", async (request, response) => {
    try {
      ...
      await settingsStore.update({
        ...
        mcpCircleciEnabled: request.body?.mcpCircleciEnabled,
        mcpCircleciToken: request.body?.mcpCircleciToken,
        mcpAirtableEnabled: request.body?.mcpAirtableEnabled,
        mcpAirtableApiKey: request.body?.mcpAirtableApiKey,
        mcpDatadogEnabled: request.body?.mcpDatadogEnabled,
        mcpDatadogApiKey: request.body?.mcpDatadogApiKey,
        mcpDatadogAppKey: request.body?.mcpDatadogAppKey,
        mcpDatadogSite: request.body?.mcpDatadogSite,
  ...
  app.post("/api/research/projects/:name/autopilot/run", async (request, response) => {
    try {
      const libraryRoot = settingsStore.settings.wikiPath;
      if (!libraryRoot) {
        response.status(503).json({ error: "Library path is not configured." });
      ...
      const message = error?.message || "Could not run research autopilot.";
      response.status(/invalid project name/i.test(message) ? 400 : 500).json({ error: message });
    }
  });

  app.post("/api/research/projects/:name/briefs/:slug/compile", async (request, response) => {
    try {
      const libraryRoot = settingsStore.settings.wikiPath;
      if (!libraryRoot) {
        response.status(503).json({ error: "Library path is not configured." });
      ...
      const body = request.body && typeof request.body === "object" && !Array.isArray(request.body)
        ? request.body
        : {};
      const moveSlugs = Array.isArray(body.moveSlugs)
        ? body.moveSlugs
        : body.only
          ? [body.only]
          : [];
      ...
      const phaseResult = result.dryRun
        ? null
      ...
      const statusCode = error?.code === "ENOENT"
        ? 404
        : /invalid project name/i.test(message)
          ? 400
          : /brief|queue|candidate|slug|cap|already/i.test(message)
            ? 400
            : 500;
