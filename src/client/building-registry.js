...
const CORE_BUILDING_MANIFESTS = [
  ...
  },
  {
    ...
    name: "LeRobot",
    category: "Robotics",
    description: "Operate a real SO-101 arm workflow end-to-end: ordering, assembly, teleop recording, and ACT policy training.",
    icon: Bot,
    ...
    onboarding: {
      variables: [
        { label: "Hugging Face account", value: "for dataset/policy repo IDs and model hosting", required: true },
        { label: "SO-101 order status", value: "ordered and shipped with recorded tracking number", required: true },
        { label: "Assembly state", value: "assembled and motors configured per LeRobot docs", required: true },
        { label: "Robot connection", value: "leader/follower arms connected and visible on local serial ports", required: true },
        { label: "Teleop data target", value: "HF dataset repo_id for recorded demonstrations", required: true },
        { label: "Training hardware", value: "single GPU (or approved cloud GPU) for ACT training", required: true },
      ],
      steps: [
        { title: "Confirm procurement", detail: "Order SO-101 parts and keep the tracking number in the project notes before setup starts." },
        { title: "Assemble and wire", detail: "Follow the SO-101 assembly docs, configure motors, then connect leader and follower arms over USB." },
        { title: "Record teleop data", detail: "Run teleoperation and collect quality demonstrations into a named Hugging Face dataset repo." },
        { title: "Train ACT policy", detail: "Train and evaluate an ACT policy on the recorded dataset, then publish the policy artifact.", completeWhen: { type: "installed" } },
      ],
    },
    agentGuide: {
      summary: "Use LeRobot when an agent must run the real SO-101 hardware loop from setup through teleop data recording and ACT policy training with Hugging Face-hosted artifacts.",
      useCases: [
        "Follow the SO-101 hardware lifecycle from order tracking to assembled and connected arm control.",
        "Record teleoperated demonstrations and push a dataset to the Hugging Face Hub.",
        "Train, evaluate, and publish an ACT policy for a concrete SO-101 task.",
        "Diagnose robot-port, camera, or runtime setup issues before expensive data collection or training runs.",
      ],
      setup: [
        "Read Hugging Face LeRobot docs first; keep hardware-specific notes in the Library for handoff.",
        "Before any arm motion, verify clear workspace, emergency stop reachability, and approved human supervision.",
        "Treat purchasing and cloud-GPU spend as approval-gated actions.",
        "Record the dataset repo_id, policy repo_id, command, ports, camera config, and commit whenever reporting results.",
      ],
      commands: [
        ...
        { label: "Evaluate ACT on robot", command: "lerobot-record --robot.type=so101_follower --robot.port=/dev/ttyACM0 --policy.path=${HF_USER}/act_so101_policy --dataset.repo_id=${HF_USER}/eval_act_so101 --dataset.num_episodes=10 --dataset.single_task=\"<task-eval>\"", detail: "Runs policy inference and records evaluation rollouts." },
      ],
      env: [
        { name: "HF_TOKEN", detail: "Hugging Face access token for pushing datasets/policies; keep secret.", required: true },
        { name: "HF_USER", detail: "Hugging Face username or org used in dataset and policy repo IDs.", required: true },
        { name: "CUDA_VISIBLE_DEVICES", detail: "Optional GPU pinning for ACT training jobs.", required: false },
      ],
      docs: [
        { label: "LeRobot docs index", url: "https://huggingface.co/docs/lerobot/en/index" },
        { label: "Installation", url: "https://huggingface.co/docs/lerobot/en/installation" },
        { label: "Getting started with real robots", url: "https://huggingface.co/docs/lerobot/main/getting_started_real_world_robot" },
        { label: "SO-101 guide", url: "https://huggingface.co/docs/lerobot/en/so101" },
        { label: "Human-in-the-loop data collection", url: "https://huggingface.co/docs/lerobot/main/hil_data_collection" },
        { label: "ACT policy guide", url: "https://huggingface.co/docs/lerobot/main/en/act" },
      ],
    },
  },
  {
