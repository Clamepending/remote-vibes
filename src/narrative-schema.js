...
//
// Returns entries in **insertion order** (state.order). The server is the
// single ordering authority: it stamps each entry with a monotonic seq via
// allocateSeq(), and that order is what's reflected in state.order on the
// client. There is NO render-time sort. This is a deliberate change from
// the timestamp-primary sort that previously lived here — the symptom
// driving the rewrite was that the user's just-sent message would render
// BELOW the assistant response when the provider's payload timestamp came
// in earlier than the user-echo timestamp (clock skew between client
// optimistic stamps and server stamps, plus Claude preserving provider
// timestamps). Insertion order, owned by the server's seq counter, is the
// only ordering rule the client respects now. opencode does the same.
